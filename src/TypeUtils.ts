...
export namespace List {
  ...
  export type Unshift<A, X> = [X, ...List.Assert<A>];
...
export namespace Num {
  ...
  export type IsNegative<N> = String.DoesStartWith<Num.ToString<N>, "-">;
  export type Negate<N> = N extends 0
    ? 0
    : IsNegative<N> extends true
    ? Num.FromString<String.Shifted<Num.ToString<N>>>
    : Num.FromString<`-${Num.Assert<N>}`>;

  export type ToString<X> = `${Num.Assert<X>}`;
  export type FromString<S> = S extends "0"
    ? 0
    : String.DoesStartWith<S, "-"> extends false
    ? {
